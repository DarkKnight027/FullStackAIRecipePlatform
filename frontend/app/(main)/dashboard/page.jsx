    ...
    <div className='min-h-screen bg-stone-50 pt-24 pb-16 px-4 md:px-8'>
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          {/* KITCHEN HEALTH SIDEBAR (High Contrast Fix) */}
          <div className="lg:col-span-4">
            <Card className="border border-stone-200 shadow-xl bg-white rounded-3xl h-full flex flex-col overflow-hidden">
                ...
                <CardContent className="flex-grow p-6 space-y-8">
                    {/* Pantry Stock Progress (Vivid Orange) */}
                    <div>
                        ...
                        <div className="h-4 bg-stone-100 rounded-full overflow-hidden border border-stone-200 p-[2px]">
                            <div className="h-full bg-gradient-to-r from-orange-500 to-orange-700 rounded-full transition-all duration-1000 shadow-inner" style={{ width: '72%' }} />
                        </div>
                    </div>

                    {/* Quick Stats Grid (Deeper Borders) */}
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-stone-50 p-4 rounded-2xl border-2 border-stone-100 text-center group hover:border-orange-300 hover:bg-white transition-all duration-300">
                            <p className="text-stone-500 text-[10px] font-black uppercase tracking-widest mb-1">Recipes</p>
                            <p className="text-3xl font-black text-stone-900 group-hover:text-orange-600">12</p>
                        </div>
                        <div className="bg-stone-50 p-4 rounded-2xl border-2 border-stone-100 text-center group hover:border-orange-300 hover:bg-white transition-all duration-300">
                            <p className="text-stone-500 text-[10px] font-black uppercase tracking-widest mb-1">Ingredients</p>
                            <p className="text-3xl font-black text-stone-900 group-hover:text-orange-600">48</p>
                        </div>
                    </div>

                    {/* Activity Visualization (Darker Bars for Visibility) */}
                    <div className="pt-2">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <span className="text-[10px] font-bold text-stone-400">Feb 1-7</span>
                        </div>
                        <div className="flex items-end justify-between h-16 gap-2 px-1">
                            {[40, 70, 45, 90, 65, 30, 85].map((height, i) => (
                                <div 
                                    key={i} 
                                    className="w-full bg-stone-200 rounded-t-md hover:bg-orange-500 transition-all duration-300 cursor-pointer shadow-sm relative group/bar" 
                                    style={{ height: `${height}%` }}
                                >
                                    <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-stone-800 text-white text-[9px] px-1 rounded opacity-0 group-hover/bar:opacity-100 transition-opacity">
                                        {height}%
                                    </div>
                                </div>
                            ))}
                        ...
                        <div className="flex justify-between mt-2 px-1">
                             {['M','T','W','T','F','S','S'].map((day, i) => (
                                 <span key={i} className="text-[9px] font-black text-stone-400 w-full text-center">{day}</span>
                             ))}
                        </div>
                    </div>
                </CardContent>
                ...
                <div className="p-4 bg-stone-50 border-t border-stone-200 text-center">
                    <button className="text-[11px] font-black uppercase tracking-widest text-orange-600 hover:text-orange-700 transition-colors flex items-center justify-center w-full gap-2 group">
                        Analytical Report <ArrowRight className="w-3 h-3 group-hover:translate-x-1 transition-transform" />
                    </button>
                </div>
            </Card>
          </div>
        ...
        {/* FULL WIDTH SECTIONS (CATEGORIES & CUISINES) */}
        {/* ... (Keep the rest of the code from the previous response for Categories and Cuisines) ... */}
            ...
            <div className="flex flex-wrap gap-4 justify-start">
              {categories.map((category) => (
                <Link key={category.strCategory} href={`/recipes/category/${category.strCategory.toLowerCase()}`}>
                  <Badge variant="secondary" className="bg-stone-50 text-stone-800 border-stone-100 hover:bg-orange-600 hover:text-white transition-all font-bold px-5 py-3 flex items-center gap-3 rounded-2xl shadow-sm">
        ...
        <section className="space-y-6 w-full">
          ...
          <Card className="border-none shadow-md bg-white rounded-[2.5rem] p-8">
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-6">
              {areas.map((area) => (
                <Link key={area.strArea} href={`/recipes?area=${area.strArea}`} className="group">
                  <div className="flex flex-col items-center justify-center p-4 rounded-3xl hover:bg-orange-50/50 transition-all border border-transparent hover:border-orange-100">
